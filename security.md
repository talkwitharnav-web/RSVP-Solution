...
### Plaintext sender passwords

**Status:** Accepted by product owner on 2026-07-29.

Sender passwords are intentionally stored in `users.raw_password`, returned by
the localhost-only admin database API, and displayed in the admin console. The
normal bcrypt hash is stored as well, but that does not protect a password when
the original plaintext is retained beside it.

Affected code:

- `src/lib/db.ts`
- `src/app/api/sender/register/route.ts`
- `src/app/api/users/[id]/password/route.ts`
- `src/app/api/dev/db/route.ts`
- `src/app/admin/db/page.tsx`

Impact: anyone who obtains database access or admin-console access can read all
sender passwords and may also compromise unrelated services where a sender
reused the same password.

Required compensating controls while this remains:

- PostgreSQL must never be publicly reachable.
- Admin routes must remain localhost-only and must not rely on a spoofable
  proxy header.
- Database backups, exports, logs, and snapshots must be treated as containing
  plaintext credentials.
- Production sender accounts should use passwords unique to this application.
- Access to `raw_password` should be logged if an audit trail is introduced.
...
`docker-compose.yml` publishes `5432:5432` and configures the database as
`postgres/postgres`. The effective Compose configuration and a live wildcard
PostgreSQL listener were verified during the audit.

Impact: if port 5432 is reachable from an attacker, they can directly read,
alter, or erase the database. Because plaintext passwords are intentionally
stored, this also becomes immediate credential disclosure.

Remediation:

- Do not publish PostgreSQL in production; place it on a private container or
  provider network.
- If local host access is needed, bind explicitly to `127.0.0.1` rather than
  all interfaces.
- Replace the default credentials with generated secrets.
- Use a restricted application role instead of the PostgreSQL superuser.
- Restrict network access with host/cloud firewall rules.

Regression concern: local database tools will need a loopback-only port mapping
...
`npm start` runs `node server.js` without setting `NODE_ENV=production`.
`server.js` therefore considers the process a development server unless the
environment was configured separately. In non-production mode,
`src/lib/session.ts` permits the committed fallback signing key when
`SESSION_SECRET` is missing.

An attacker who knows that key can create valid admin or sender cookies. Public
receiver pages also pass the complete event row to a client component,
including the owner's `created_by` UUID, even though the public event API
deliberately strips that field. Together, these allow targeted sender-session
forgery if the fallback key is active.

Impact: sender impersonation, access to private RSVP statistics, and
modification or deletion of another sender's invitations. A forged admin token
also combines with S3 below.

Remediation:

- Add an explicit production launcher that sets production mode.
- Refuse to start any network-accessible process without a strong
  `SESSION_SECRET`, regardless of `NODE_ENV`.
- Generate local development secrets instead of using a shared committed key.
- Pass a public event projection, not a raw database row, into
  `GuestEventView`.

Regression concern: first-run development needs to generate or request a local
secret instead of silently starting with a universal fallback.
...
`DELETE /api/events/[slug]` reads and verifies `admin_session` directly. It
does not call `requireAdmin()`, which is the helper that also verifies the
request came from localhost. The proxy cannot protect this route because GET,
PUT, and DELETE share the same public path.

Impact: a stolen or forged admin cookie can delete an arbitrary known event
from a remote address.

Remediation: make the admin branch call `requireAdmin()` while retaining the
existing sender ownership path for normal sender deletion.

Regression concern: sender-owned deletion must continue to work from LAN or a
...
`bodyTooLarge()` only checks the declared `Content-Length`. A request using
chunked transfer encoding has no declared length and is fully buffered by
`req.json()`. A harmless 70 KB chunked login body was accepted and parsed,
returning 401 instead of the intended 413. `/api/logout` has no size check at
all.

Impact: unauthenticated attackers can send very large bodies and exhaust Node
memory or keep workers occupied.

Remediation:

- Enforce request limits while streaming at the HTTP server or trusted reverse
  proxy, before Next.js parses the body.
- Reject unsupported transfer/content types where practical.
- Add the same bounded-body handling to every JSON route, including logout and
  admin seed/purge routes.

Regression concern: event and design routes legitimately accept larger bodies
than credential routes and must retain a separate controlled media limit.
...
The upgrade handler in `server.js` constructs a URL from the untrusted `Host`
header outside a `try/catch`. An isolated test confirmed that Node passes a
malformed value such as `Host: [` into the upgrade callback and `new URL()`
throws `ERR_INVALID_URL`. No process-level handler catches it.

Impact: one malformed unauthenticated upgrade request can stop the Node
process.

Remediation: parse against a fixed safe base or catch URL-construction errors
and destroy the offending socket.

Regression concern: non-RSVP upgrades must still be delegated to Next.js for
...
The WebSocket server has a global 200-client cap but no per-IP cap. A raw
WebSocket client can set an arbitrary matching `Origin`, so the browser-origin
check is not authentication. One source can occupy every slot. The `ws`
library also defaults to a 100 MB inbound message limit, while this application
expects clients to send no messages at all.

Impact: an attacker can deny real-time service, force large memory allocation,
or repeatedly reconnect to consume process resources.

Remediation:

- Set a very small `maxPayload`.
- Close connections that send application messages.
- Add per-IP connection and handshake-rate limits.
- Keep the global cap and liveness cleanup as defense in depth.

Regression concern: browser ping/pong behavior and ordinary reconnects must
continue to work.
...
`sanitizeDesignConfig()` only checks that `canvasJSON` is an object containing
an `objects` array and is below the total JSON size limit. The guest renderer
then passes it directly to Fabric's `loadFromJSON()`. Fabric instantiates
registered classes selected by each object's `type`, recursively loads groups
and filters, and loads arbitrary image `src` values through an HTML image
element.

Impact: a malicious sender can make a guest's browser contact an external or
internal URL, track invitation views, or become slow/unresponsive through
large object counts, nested groups, filters, or images.

Remediation:

- Define a server-side allowlist of supported Fabric object types and fields.
- Limit total objects, nesting depth, text length, path complexity, and filter
  count/cost.
- Permit image sources only when they are accepted, size-bounded data URLs.
- Reject or remove unknown properties instead of passing the raw scene to
  Fabric.

Regression concern: all five templates, per-character text styles, groups,
icons, decorations, uploaded images, and old saved designs need round-trip
tests against the stricter schema.
...
Registration is public and authenticated users can create unlimited events.
Each event may contain multi-megabyte image/design data. There is no per-user
event count, storage quota, or authenticated mutation rate limit.

Every write also sends a global `db-changed` event. Unrelated open receiver,
editor, gallery, statistics, and admin pages then refetch data. The admin API
returns full unpaginated rows, including large inline media.

Impact: one or more free accounts can fill database storage and amplify a
write loop into many database reads and large API responses.

Remediation:

- Add per-account event and stored-byte quotas.
- Rate-limit authenticated creates/updates separately from login limits.
- Paginate admin and sender listings and omit large media from summaries.
- Broadcast event-specific identifiers and refetch only affected resources.
- Coalesce repeated refresh notifications.

Regression concern: legitimate large invitations and live updates for the
affected event must continue to work.
...
Sessions are stateless signed tokens valid for up to 30 days. Logout removes
the current browser's cookie but does not invalidate the token. Resetting a
sender's password does not change or revoke existing tokens either.

Impact: someone who copied a session cookie remains logged in after the victim
logs out or changes their password.

Remediation: introduce stored session IDs or a per-user `session_version` and
verify it in `requireSender()`. Revoke sessions on password reset and provide a
defined logout-all-devices operation.

Regression concern: decide whether an ordinary logout should affect one device
...
The intended dummy bcrypt hash is malformed for `bcryptjs` and is rejected
almost immediately. Audit measurements were approximately 1 ms for an unknown
username versus 65 ms for a real account with a wrong password.

Impact: a remote attacker can discover valid usernames before attempting
credential stuffing. The existing per-IP rate limit slows but does not remove
the signal.

Remediation: replace the dummy value with a real bcrypt hash generated at the
same configured cost and add a timing/work regression test.

Regression concern: authentication responses and successful login behavior
must remain unchanged.

## Deployment-Dependent Risks Requiring Verification
...
That is correct while clients connect directly. If a reverse proxy on the same
machine forwards public traffic to Node, every public request appears to come
from loopback.

Potential impact: admin routes become publicly reachable, localhost-only health
details are exposed, and all clients share one rate-limit bucket.

Required production decision: define the proxy topology and accept forwarding
headers only from explicitly trusted proxy addresses. Do not trust arbitrary
...
only when `FORCE_SECURE_COOKIES=true`. No production reverse proxy, HTTPS
redirect, HSTS policy, or deployment configuration exists in the repository.

Potential impact: session cookies can be intercepted over an untrusted network
if the app is deployed over HTTP or the secure-cookie flag is omitted.

Required production decision: terminate TLS at a defined trusted boundary,
redirect HTTP to HTTPS, enable secure cookies, and add HSTS after HTTPS is
...
`npm audit --omit=dev` reported three high advisories through Next.js:

- PostCSS 8.4.31: source-map path/file disclosure advisories.
- sharp 0.34.5: inherited libvips image-processing advisories.

No route was found that accepts attacker-controlled CSS for server-side PostCSS
processing, and this application performs uploaded-image conversion in the
browser rather than explicitly calling sharp on the server. Application-level
exploitability was therefore not confirmed.

Remediation: track a compatible Next.js release containing patched transitive
versions. Do not apply npm's suggested forced downgrade to Next 9.
...
- Public RSVP submission intentionally allows duplicate anonymous responses.
  The current IP limiter slows basic spam but does not enforce one response per
  guest or invitation token. Whether stronger identity is needed is a product
  decision.
- Public event slugs use 8 random characters from a 31-character alphabet,
  which is not sequential or trivially guessable, but published links remain
  bearer links and should be treated as public once shared.

## Controls Inspected And Found Effective

- SQL statements that consume request values are parameterized; no SQL
  injection path was found.
- Sender event and RSVP-statistics routes generally verify `created_by`
  ownership server-side.
- Unpublished invitations are hidden from non-owners at both page and API
  levels.
- React escaping is used for ordinary stored text; no user-controlled raw HTML
  rendering was found.
- External RSVP links are restricted to HTTP/HTTPS at storage and render time.
- SVG image uploads are rejected.
- Session signatures use HMAC-SHA256, constant-time signature comparison, and
  an enforced expiry when a private secret is configured.
- `requireSender()` checks that the referenced user still exists, preventing
  deleted-account ghost sessions from continuing to call protected APIs.
- Usernames use exact case-insensitive SQL equality rather than wildcard
  `ILIKE` matching.
...
The repository contains no final production hosting configuration. The audit
could not conclusively assess cloud firewalls, managed database controls,
reverse-proxy behavior, TLS certificates, secret entropy and rotation, backup
access, production logs, monitoring, alerting, or incident-response processes.

Git history contains an earlier admin-credential implementation. The active
