...
- `nanoid` for short public slugs, `qrcode` installed but not yet wired into any page, `lucide-react` for all icons (never emoji — see UI and Design Rules).
- Dev server: `npm run dev`, pinned to **port 3001**, not the Next.js default 3000 — the user's other project (restaurant order tracker) already runs on 3000, and both projects may run side by side. `next start` is also pinned to 3001. All scripts (`scripts/*.sh`, `scripts/*.ps1`) and docs reference 3001.
- No custom server (`server.js`) yet — plain `next dev` is fine, nothing needs a raw WebSocket/HTTP upgrade yet.
...
| Route | Purpose |
|---|---|
| `/` | **Admin gateway page** (`AdminGatewayPage`). Sidebar nav (RSVP Sender / RSVP Receiver / Access DB, all dud links to `/admin/sender`, `/admin/receiver`, `/admin/db` which don't exist yet), Log Out button, and a combined "Healthy" status + theme-toggle pill in the header. This is the canonical root — not the old create-flow landing page. |
| `/admin` | Redirects (307) to `/`. Exists so `/admin` still resolves somewhere sensible; future real subroutes (`/admin/db`, `/admin/sender`, `/admin/receiver`) will live under this prefix as actual pages, not redirects. |
...
- Theme is implemented per `theme.md` (full research/rationale/token table lives there — this is just the "what's wired in" summary). Fonts: Bricolage Grotesque (`--font-display`, headings only) + Plus Jakarta Sans (`--font-body`, everything else), loaded via `next/font/google` in `layout.tsx`. Colors: full light/dark CSS variable set in `globals.css` (`--color-bg-base`, `--color-bg-raised`, `--color-text-primary`, `--color-text-muted`, `--color-accent-coral`/`-coral-text`/`-on-coral`, `--color-accent-sage`, `--color-accent-lavender`, `--color-border`, `--color-danger`), switched via `prefers-color-scheme` (no manual toggle wired up yet — the theme-toggle button on `/` is currently a dud, same as the nav links).
- **Icons are always `lucide-react`, never emoji.** Emoji was tried first on the admin page and explicitly rejected by the user (2026-07-23) — emoji ignore theme color tokens (fixed rendering regardless of light/dark mode) and read as low-effort next to a real icon set. `lucide-react` is the same package already used successfully on the reference project.
- CSS custom properties (`--color-*`), never literal Tailwind color utility classes in components — same convention as the reference project, keeps light/dark swappable from one place.
